...
bool Operation_anyOf::validate(std::stringstream& msg, const std::string& pathPrefix) const
{
    bool success = true;
    const std::string _pathPrefix = pathPrefix.empty() ? "Operation_anyOf" : pathPrefix;

    
    if (m_value == Operation_anyOf::eOperation_anyOf::INVALID_VALUE_OPENAPI_GENERATED)
    {
        success = false;
