...
bool DataFormat_anyOf::validate(std::stringstream& msg, const std::string& pathPrefix) const
{
    bool success = true;
    const std::string _pathPrefix = pathPrefix.empty() ? "DataFormat_anyOf" : pathPrefix;

    
    if (m_value == DataFormat_anyOf::eDataFormat_anyOf::INVALID_VALUE_OPENAPI_GENERATED)
    {
        success = false;
