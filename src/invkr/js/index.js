...
function onboardInvoker(public_key, notif_dest) {
  return new Promise((resolve, reject) => {
    ...
    let newApiInvokerEnrolmentDetils =
      APIInvokerEnrolmentDetails.constructFromObject({
    ...
    let callback = (error, data, response) => {
      if (error) {
        ...
        console.log(
          `[CAPIF][API Invoker] Onboarded; My Invoker ID is: ${invkr_id}`
        );
        resolve(invkr_id);
      }
    };
    my_default_api.apiInvokerManagementV1OnboardedInvokersPost(
      newApiInvokerEnrolmentDetils,
      callback
    );
  });
}
